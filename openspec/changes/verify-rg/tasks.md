...
**Depends on**: nothing
**Deliverables**:
- [ ] `test_rg_scaling_invariance` — for α ∈ {2.0, 10.0, 0.1}, assert `Rg(α·coords, α·radii) == α·Rg(coords, radii)` with `assert_relative_eq!(..., epsilon = 1e-10)`
- [ ] `test_rg_translation_invariance` — translate all coords by `[17.3, -4.1, 9.0]`, assert Rg unchanged with tolerance `1e-10`
- [ ] `test_rg_dimer_closed_form` — 2 spheres `r=1` separated by distance 2, assert `Rg == sqrt(1.0 + 3.0/5.0)` with tolerance `1e-10`
- [ ] `test_rg_linear_chain_matches_kf_analytic` — for N ∈ {3, 5, 10} build chain of touching unit spheres, compare to `kf_analytic::radius_of_gyration(Line, n, 2.0)`, tolerance `1e-6`
- [ ] `test_rg_hex_plane_matches_kf_analytic` — for N ∈ {7, 19} build centred-hex planar arrangement, compare to `kf_analytic::radius_of_gyration(Hex, n, 2.0)`, tolerance `1e-6`
- [ ] Also cover existing scenarios from spec if not already present: single sphere `r` → `sqrt(3/5)·r`, empty input → `0.0`
...
**Depends on**: nothing
**Deliverables**:
- [ ] Module constants: `PARAM_KEY_DIAMETER = "primary_particle_diameter_nm"`, `PARAM_KEY_RADIUS_LEGACY = "primary_particle_radius_nm"`, `DEFAULT_DIAMETER_NM = 50.0`, `SCHEMA_VERSION_CURRENT = "v2"`
- [ ] `get_primary_particle_diameter_nm(params: dict) -> float` — fallback order: v2 key if positive → v1 key × 2 if positive → `DEFAULT_DIAMETER_NM`
- [ ] `get_scale_factor_nm(params: dict) -> float` — returns `get_primary_particle_diameter_nm(params) / 2`
- [ ] `get_schema_version(params: dict) -> str | None` — returns `"v2"` | `"v1"` | `None`; `"v1"` when `primary_particle_radius_nm` present and no explicit version; `None` for fully ambiguous rows
- [ ] Unit tests covering: v2 present → D; only v1 present → R×2; neither → 50.0; both present → v2 wins; v1 = 0 → default; v1 negative → default; v2 = 0 → fall through to v1; explicit `parameters_schema_version="v1"` detected
...
- [ ] `export function getPrimaryParticleDiameterNm(params: Record<string, unknown>): number` — same fallback order as Python
- [ ] `export function getScaleFactorNm(params: Record<string, unknown>): number` — returns `dpo / 2`
- [ ] `export function getSchemaVersion(params: Record<string, unknown>): "v1" | "v2" | null`
- [ ] Unit tests mirroring T2: v2 present; only v1; neither; both (v2 wins); v1=0; v1<0; v2=0 falls through; explicit v1 string
...
**Depends on**: T2
**Deliverables**:
- [ ] Import `get_scale_factor_nm` from `apps.simulations.services.params`
- [ ] Compute `scale = get_scale_factor_nm(simulation.parameters)` once per export
- [ ] Multiply the Rg value by `scale` before writing to CSV
- [ ] Ensure `Unit` column exists and Rg row has `Unit = "nm"` (replace any prior `"particle radii"`)
- [ ] No behavioural change for non-Rg rows
...
**Depends on**: T2
**Deliverables**:
- [ ] Per simulation in the batch, compute `scale = get_scale_factor_nm(sim.parameters)` (shim handles v1/v2)
- [ ] Multiply each Rg column value by the per-row `scale`
- [ ] Add a `Unit` column (or per-column unit annotation) so Rg columns are labelled `"nm"`. This is additive — existing columns preserved
- [ ] Verify both CSV emission sites (~1116 and ~1143) go through the same path
...
**Deliverables**:
- [ ] On create, ensure `validated_data["parameters"]["parameters_schema_version"] = "v2"`
- [ ] On create, ensure `validated_data["parameters"]` carries `primary_particle_diameter_nm` and NEVER writes `primary_particle_radius_nm`
- [ ] If the serializer accepts legacy payloads from old clients (transitional), it MUST convert radius → diameter on the way in before persisting
- [ ] Add or extend a serializer test asserting a newly created simulation persists `parameters_schema_version == "v2"` and only the new key
...
**Depends on**: T2
**Deliverables**:
- [ ] Replace any direct `params["primary_particle_radius_nm"]` or `params["primary_particle_diameter_nm"]` read with `get_primary_particle_diameter_nm(params)` (or `get_scale_factor_nm`, whichever matches the downstream unit)
- [ ] Verify the engine input values produced are numerically unchanged for v2 simulations and correctly doubled-from-radius for v1 simulations
- [ ] Add or extend a task-level test (or targeted unit test on the mapping helper) covering both schema versions
...
**Depends on**: T3
**Deliverables**:
- [ ] Replace inline `params.primary_particle_radius_nm` / inline scale math with `getScaleFactorNm(simulation.parameters)` imported from `@/lib/units`
- [ ] Ensure the displayed Rg is `rg_engine × scale` and the label/adjacent text includes ` nm`
- [ ] No duplicate scaling (multiply exactly once at this boundary)
...
**Depends on**: T3
**Deliverables**:
- [ ] Import and apply `getScaleFactorNm` per-simulation
- [ ] Append ` nm` or `(nm)` to Rg cells
- [ ] Same numeric result as the detail page for the same simulation
...
**Depends on**: T3
**Deliverables**:
- [ ] Multiply Rg by `getScaleFactorNm(params)` at the display site
- [ ] Append ` nm` to the value
- [ ] Verify the AI prompt/context (if it also receives Rg) uses the same scaled value OR explicitly the dimensionless engine value — do NOT send conflicting numbers
...
**Depends on**: T3
**Deliverables**:
- [ ] Column header literal changes to `Rg (nm)` (or localisable equivalent)
- [ ] Rg cell value = `rg_engine × getScaleFactorNm(row.parameters)`
- [ ] Sort/compare logic (if any) uses the scaled value consistently
...
**Deliverables**:
- [ ] `yData = rg_evolution.map(v => v * scale)` where `scale = getScaleFactorNm(params)`
- [ ] Y-axis label: `log10(Rg/nm)`
- [ ] X-axis unchanged
- [ ] Empty `rg_evolution` renders "no data" gracefully (no NaN/Infinity)
...
**Depends on**: T3
**Deliverables**:
- [ ] Form state key renamed `primary_particle_radius_nm` → `primary_particle_diameter_nm`
- [ ] Default value: `25`
- [ ] Label: "Primary Particle Diameter (nm)" or equivalent
- [ ] Help text: `"Primary particle diameter (dpo) in nm (soot convention)"`
- [ ] Submission payload writes `primary_particle_diameter_nm` and never `primary_particle_radius_nm`
- [ ] Any validation (min/max) adjusted for a diameter rather than radius
- [ ] Remove any now-dead imports or constants tied to the old key
...
**Depends on**: T3
**Deliverables**:
- [ ] Props: `{ simulationId: string; schemaVersion: "v1" | null; userId: string; onDismiss: () => void }`
- [ ] Renders only when `schemaVersion` is `"v1"` or `null` AND no dismissal flag is present
- [ ] Dismissal key: `localStorage["dismissed-banner:unit-convention:" + userId]` (boolean)
- [ ] Copy (EN): "Unit convention updated. Rg values previously displayed were 2× the correct nm value; display is now corrected. Stored data unchanged." + link "Learn more" → `/docs/unit-convention` + "Dismiss" button
- [ ] `onDismiss` writes the localStorage key and calls parent callback
- [ ] SSR-safe: guard `localStorage` access behind `typeof window !== "undefined"`
...
**Depends on**: T10d, T3
**Deliverables**:
- [ ] Detail page: mount `<UnitConventionBanner />` at top, pass `schemaVersion = getSchemaVersion(simulation.parameters)` (cast/narrow to `"v1" | null` since v2 returns early)
- [ ] Project list page: mount once at top of page (not per-row) if ANY listed simulation resolves to v1/null schema
- [ ] Pass current `userId` from the auth context
...
**Depends on**: T10d
**Deliverables**:
- [ ] Renders when `schemaVersion = "v1"` and no localStorage flag
- [ ] Renders when `schemaVersion = null` and no localStorage flag
- [ ] Does NOT render when `schemaVersion = "v2"` (component should early-return or the prop type guards it)
- [ ] Does NOT render when localStorage dismissal flag present for the given `userId`
- [ ] Dismiss click: writes localStorage key, invokes `onDismiss`, hides banner
- [ ] Different `userId` has independent dismissal state
...
**Deliverables**:
- [ ] Fixture: v1 simulation (`parameters.primary_particle_radius_nm = 25.0`, no schema version) with a known `rg_engine`
- [ ] Fixture: v2 simulation (`parameters.primary_particle_diameter_nm = 50.0`, `parameters_schema_version = "v2"`) with a known `rg_engine`
- [ ] Single-sim CSV test: for both fixtures, assert Rg cell equals `rg_engine * (D_nm/2)` and `Unit` column cell equals `"nm"`
- [ ] Batch CSV test: mixed-version batch, assert each row scaled per its own shim resolution, Unit annotation present
- [ ] Edge case: v1 simulation with `primary_particle_radius_nm = 0` → shim defaults → export still succeeds with `nm` unit
...
- [ ] Section 1: "Engine is dimensionless" — state that `metrics.radius_of_gyration` is unitless
- [ ] Section 2: "Display scaling" — explain `Rg_display_nm = Rg_engine × (primary_particle_diameter_nm / 2)` and that the single helper lives in `services/params.py` (backend) and `lib/units.ts` (frontend)
- [ ] Section 3: "Schema versioning" — describe `parameters_schema_version`, v1 (legacy, `primary_particle_radius_nm`) vs v2 (`primary_particle_diameter_nm` + explicit version stamp), and the shim fallback order
- [ ] Section 4: "Adding a new display surface" — checklist for contributors: import the helper, multiply exactly once, label `nm`, do not re-scale downstream
...
**Deliverables**:
- [ ] `cargo test -p aglogen-engine` green (includes T1)
- [ ] `pytest backend` green (includes T2 shim tests + T6 serializer test + T6b mapping test + T11 integration tests)
- [ ] Frontend unit tests green (includes T3 shim tests + T10f banner test)
- [ ] Lint + typecheck green on backend and frontend
- [ ] Manual acceptance checklist (T12) attached to the PR
- [ ] Changelog entry drafted noting: CSV `Unit` now `"nm"`, Rg display scales by `dpo/2`, legacy `primary_particle_radius_nm` still read, new writes use `primary_particle_diameter_nm` + schema v2
