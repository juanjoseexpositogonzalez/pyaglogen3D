...
- [ ] T2.1 — Replace single-axis rotation with uniform spherical sampling (azimuth φ∈[0,2π), elevation θ=arcsin(U(−1,1))) for initial CoM direction in `position_clusters_for_contact` (file: `aglogen_core/engine/src/aggregation/tunable_cc.rs`)
- [ ] T2.2 — Add `max_merge_retries` field (default 100) to `CcTunableConfig` / `TunableCcParams` struct (file: `aglogen_core/engine/src/aggregation/tunable_cc.rs`)
