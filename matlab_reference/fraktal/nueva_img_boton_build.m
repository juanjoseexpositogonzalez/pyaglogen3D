function fig_hdl = nueva_img_boton_build
% DATOS_IMAGENES_BUILD
%-------------------------------------------------------------------------------
% File name   : nueva_img_boton_build.m
%-------------------------------------------------------------------------------
...
% Creation of all uicontrols
		...
		handles.dpo_mat_texto = uicontrol( ...
			...
			'Style', 'text', ...
			'Units', 'characters', ...
			'Position', [21 23 32.2 3.30769230769231], ...
            'FontSize', 12, ...
            'FontName', 'Candara', ...
            'Foregroundcolor', [1 0.35 0],...
			'String', 'diámetro medio de las partículas');
	...
	function sub_img_boton_Callback(hObject,evendata) %#ok<INUSD>
        if handles.primeravezi==1
            [handles.imagen,handles.dir_img_ind]=uigetfile({'*.jpg',...
                'Archivo JPG';'*.png','Archivo PNG';'*.tiff',...
                'Archivo TIFF';'*.bmp','Archivo BMP'},'Seleccione una imagen');
        else
            [handles.imagen,handles.dir_img_ind]=uigetfile({'*.jpg',...
                'Archivo JPG';'*.png','Archivo PNG';'*.tiff',...
                'Archivo TIFF';'*.bmp','Archivo BMP'},'Seleccione una imagen',...
...
%% ---------------------------------------------------------------------------
	function guardar_datos_boton_Callback(hObject,evendata) %#ok<INUSD>
        ...
        if handles.npix<0 || handles.dpo_mat<0 || handles.imagen_cargada==0
        ...
        elseif handles.npix>=0 && handles.dpo_mat>=0
            ...
            set(handles.guardar_datos_boton,'str','TRABAJANDO...','backg',[1 .6 .6]) % Change color of button. 
            ...
            if error_igual==1
                error=errordlg('La imagen seleccionada ya ha sido cargada anteriormente','Error');                
                    kids0=findobj(error,'Type','UIControl');
                    kids1=findobj(error,'Type','Text');
                ...
                for delta=handles.delta_inicio:handles.delta_intervalo:handles.delta_fin
                    dfmat2=GeneralExe2012individual(handles.foto,handles.nfoto,handles.npixescala,handles.dpomat,0,delta);
                    handles.dfmatnueva=[handles.dfmatnueva;dfmat2];
                end
            ...
            error=errordlg('Algún dato introducido no es un número','Error');            
                kids0=findobj(error,'Type','UIControl');
                kids1=findobj(error,'Type','Text');
