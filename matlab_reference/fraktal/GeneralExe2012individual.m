function [dfmat]=GeneralExe2012individual(foto,nfoto,npixescala,dpomat,automatico,delta)
...
%       nomarchivo=strcat(nfoto{ipri},'-',int2str(jpri),'.jpg'); %CAMBIO
        nomarchivo=strcat(nfoto_(ipri),'.jpg');
        if ~exist(nomarchivo)
            nomarchivo=strcat(nfoto_(ipri),'.png');
        end
        if ~exist(nomarchivo)
            nomarchivo=strcat(nfoto_(ipri),'.bmp');
        end
        if ~exist(nomarchivo)
            nomarchivo=strcat(nfoto_(ipri),'.tif');
        end
    else
