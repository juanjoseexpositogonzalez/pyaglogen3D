...
use super::result::FractalResult;

/// Internal box-counting implementation.
pub fn box_counting_internal(
    image: &[Vec<bool>],
    min_box_size: usize,
    max_box_size: usize,
    num_scales: usize,
    ...
    let (slope, _intercept, r_squared, std_error, residuals) =
...
pub fn count_boxes(image: &[Vec<bool>], box_size: usize) -> usize {
    let height = image.len();
    let width = if height > 0 { image[0].len() } else { 0 };
...
pub fn linear_regression(x: &[f64], y: &[f64]) -> (f64, f64, f64, f64, Vec<f64>) {
    let n = x.len() as f64;
    if n < 2.0 {
