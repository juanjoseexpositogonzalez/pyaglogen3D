...
fn calculate_prefactor_coefficients(npo: f64, delta: f64) -> (f64, f64, f64) {
    ...
    let cube_root_arg =
        3.0 / 20.0 * npo + 1.0 / 120.0 * (324.0 * npo.powi(2) + 343.0 / 15.0).sqrt();
    ...
    let ckf_denom_part2 = 4.0 / delta.powi(2)
        * (alfa(delta, 12.0)
            * (7.0 / 5.0 * in_val.powi(5) - 7.0 / 2.0 * in_val.powi(4) + 4.0 * in_val.powi(3)
            ...
            + alfa(delta, 7.0) * (20.0 * in_val.powi(3) - 24.0 * in_val.powi(2) + 4.0 * in_val)
            + alfa(delta, 9.0)
...
pub fn analyze_granulated_2012(
    ...
    let (npo_visual, dpo_estimated, _avg_radius_px) =
        estimate_particles_and_dpo(binary.view(), geometry.length_per_pixel);
    ...
    let dpo = params.dpo;
    let vol_factor =
        1.0 - jf * (4.0 * delta.powi(3) - 6.0 * delta.powi(2) + 2.0) / (8.0 * delta.powi(3));
