...
pub fn run_ballistic_cc_internal(params: BallisticCcParams, seed: u64) -> SimulationResult {
    ...
    while clusters.len() > 1 && iterations < max_iterations {
        ...
        let current_distance = impactor
            .geometric_center
            .distance_to(&impacted.geometric_center);
        let launch_distance =
            current_distance + impactor.bounding_radius + impacted.bounding_radius;
        let impactor_start =
            impacted.geometric_center + offset_on_disc - trajectory_dir * launch_distance;
