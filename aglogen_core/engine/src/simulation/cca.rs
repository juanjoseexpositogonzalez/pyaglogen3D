...
pub fn run_cca_internal(params: CcaParams, seed: u64) -> SimulationResult {
    ...
    loop {
        ...
        for cluster in &mut clusters {
            ...
            let delta = Vector3::new(
                dx * step_size * mobility,
                dy * step_size * mobility,
                dz * step_size * mobility,
            );
        ...
        for i in 0..clusters.len() {
            ...
            for j in (i + 1)..clusters.len() {
                ...
                if dist < max_dist {
                    ...
                    if check_cluster_collision_pbc(
                        &clusters[i],
                        &clusters[j],
                        effective_box_size,
                        sintering_coeff,
...
fn check_cluster_collision_pbc(
    a: &Cluster,
    b: &Cluster,
    box_size: f64,
    sintering_coeff: f64,
...
mod tests {
    ...
    fn test_cca_single_agglomerate() {
        ...
        assert!(
            ...
            "Df should be > 1 for a connected agglomerate"
        );
        // Coordination should show connectivity
        assert!(
            result.coordination_mean > 0.5,
