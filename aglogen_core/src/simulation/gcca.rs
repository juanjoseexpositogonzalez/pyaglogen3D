...
fn determine_split<R: Rng>(
    ...
    match strategy {
        ...
        SplitStrategy::Stochastic { mean_ratio, std_ratio } => {
        ...
        match split_strategy {
            ...
            _ => {
                ...
                for i in 0..n_pairs {
                    ...
                    let separation =
                        compute_required_separation(&cluster1, &cluster2, rg_target)
...
/// Generalized Cluster-Cluster Aggregation (Tomchuk & Avdeev 2020)
///
/// A unified framework for fractal aggregate generation that encompasses:
/// - Symmetric CCA (Filippov): Equal cluster splits
/// - Particle-Cluster: Sequential one-particle-at-a-time growth
/// - Stochastic: Configurable split ratio distribution
///
/// # Arguments
...
/// * `split_strategy` - "symmetric", "particle_cluster", or "stochastic"
/// * `stochastic_mean_ratio` - Mean split ratio for stochastic mode (default 0.5)
/// * `stochastic_std_ratio` - Std dev of split ratio for stochastic mode (default 0.1)
...
pub fn run_gcca(
    ...
    let strategy = match split_strategy.to_lowercase().as_str() {
        "symmetric" => SplitStrategy::Symmetric,
        "particle_cluster" | "pc" => SplitStrategy::ParticleCluster,
        "stochastic" => SplitStrategy::Stochastic {
...
/// Compute structure factor S(q) for an aggregate
///
/// S(q) = 1/Np * Σ_i Σ_j sin(q|ri - rj|) / (q|ri - rj|)
///
/// For fractal aggregates: S(q) ∝ q^(-Df) in the range 1/Rg ≪ q ≪ 1/a
///
/// # Arguments
/// * `coordinates` - Particle coordinates as flat array [x1,y1,z1,x2,y2,z2,...]
...
pub fn structure_factor(
    ...
    let coords: Vec<[f64; 3]> = coordinates
        .chunks(3)
        .map(|c| [c[0], c[1], c[2]])
        .collect();
